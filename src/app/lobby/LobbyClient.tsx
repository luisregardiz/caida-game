...
export function LobbyClient({ initialTables }: LobbyClientProps) {
  ...
  const handleJoinWithCode = async (e: React.FormEvent) => {
    ...
    if (error || !data) {
      setJoinError("Código inválido, la sala ya está llena o el juego ya empezó.");
    } else {
      useGameLogicStore.getState().resetEngine();
      router.push(`/mesa/${data.id}`);
    }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black text-white">Lobby de Caída</h1>
          <p className="text-white/50 text-sm mt-1">Juega con un amigo o contra la máquina</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mt-12">
        ...
        >
          <div className="w-16 h-16 rounded-full bg-amber-500/20 flex items-center justify-center text-3xl mb-4 group-hover:bg-amber-500/30 transition-colors">
            🏠
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Crear Sala Privada</h2>
          <p className="text-white/50 text-sm">Genera un código e invita a un amigo a jugar contigo.</p>
        </motion.div>
        ...
        >
          <div className="w-16 h-16 rounded-full bg-blue-500/20 flex items-center justify-center text-3xl mb-4 group-hover:bg-blue-500/30 transition-colors">
            🔑
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Unirse con Código</h2>
          <p className="text-white/50 text-sm">Si ya tienes un código, úsalo aquí para entrar a la mesa.</p>
        </motion.div>
        ...
        >
          <div className="w-16 h-16 rounded-full bg-purple-500/20 flex items-center justify-center text-3xl mb-4 group-hover:bg-purple-500/30 transition-colors">
            🤖
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Practicar vs CPU</h2>
          <p className="text-white/50 text-sm">Juega offline contra el bot para mejorar tus habilidades.</p>
        </motion.div>
      </div>
      ...
      <AnimatePresence>
        {showJoinModal && (
          <>
            ...
            >
              <div className="glass rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                <h2 className="text-lg font-bold text-white mb-5">Unirse a Sala</h2>

                <form onSubmit={handleJoinWithCode} className="space-y-4">
                  <div>
                    <label htmlFor="join-code" className="block text-xs font-medium text-white/60 mb-1.5">
